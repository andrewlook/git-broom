...
```
git-broom                   # review implemented cleanup groups in sequence
git-broom unpushed          # only review local branches with no remote tracking
git-broom gone unpushed     # review two explicit groups in order
...
With the addition of multiple cleanup groups, the CLI should accept one or more positional mode names plus flags:

```
git-broom                    # default workflow: review all implemented groups in order
git-broom gone               # only review gone branches
git-broom unpushed           # only review unpushed branches
git-broom gone unpushed      # explicit group order
git-broom <mode>... --batch
git-broom <mode>... --dry-run
```

The default (no explicit modes) should review all implemented groups in order. Interactive mode confirms each group independently.

`--dry-run` should group output by cleanup group, for example:
...
- [x] Default interactive mode reviews all implemented groups in sequence
- [x] `git-broom gone unpushed --dry-run` groups results by cleanup group with descriptive headers
...
| User confusion between modes | Clear group label + short description in the interactive flow and dry-run headers |
